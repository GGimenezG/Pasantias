        ...
        [
            {
                ...
                title: 'Listado datos maestros de los Requerimientos de discapacidad activos',
                filename: 'Listado Requerimientos',
                exportOptions:
                    {
                        columns: [0, 1, 2]
                    }
...
function onIncluir() {
	...
	codigo();
	//<?php header('location: /uaed/Requerimiento/codigo'); ?>
	$('#descrp').val("");
	$('#nombre').val("");
...
function codigo() 
    { 
        ...
        var request = $.ajax
            ({
                url: "/uaed/Requerimiento/codigo", //¿Ese archivo se llama así realmente?
                method: "POST"
...
function activar(datos){
	$.ajax({
		url: "/uaed/Requerimiento/activar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("codigo:") != 0){
				...
				agregarFila(fila);
				mostrarMensajeExito("Requerimiento activado satisfactoriamente!!!");
...
function onAgregar() {
	var datos = $('#formRequerimiento').serialize();
	console.log(datos, "agregar");
	$.ajax({
		url: "/uaed/Requerimiento/registrar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("activar:") != 0){
				...
				var fila = JSON.parse(data);
				agregarFila(fila);
				mostrarMensajeExito("Requerimiento incluido satisfactoriamente!!!");
			}else if (data.indexOf("error:") != 0){
				mostrarMensajeError(data["error"]);
			}
	...
	$.ajax({
		...
		success: function(data) {
			if (data.indexOf("error:") != 0) {
				...
				agregarFila(cargo);
				mostrarMensajeExito("Requerimiento modificado satisfactoriamente!!!");
			}
			else {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.indexOf("codigo:") != 0) {
				...
				mostrarMensajeExito('Requerimiento eliminado satisfactoriamente!!!');
			}
			else {
