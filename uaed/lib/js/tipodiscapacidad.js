        ...
        [
            {
                ...
                title: 'Listado datos maestros de los Tipodiscapacidad de discapacidad activos',
                filename: 'Listado Tipodiscapacidad',
                exportOptions:
                    {
                        columns: [0, 1, 2]
                    }
...
function onIncluir() {
	...
	codigo();
	//<?php header('location: /uaed/Tipodiscapacidad/codigo'); ?>
	$('#descrp').val("");
	$('#nombre').val("");
...
function codigo() 
    { 
        ...
        var request = $.ajax
            ({
                url: "/uaed/Tipodiscapacidad/codigo", //¿Ese archivo se llama así realmente?
                method: "POST"
...
function activar(datos){
	$.ajax({
		url: "/uaed/Tipodiscapacidad/activar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("codigo:") != 0){
				...
				agregarFila(fila);
				mostrarMensajeExito("Tipodiscapacidad activado satisfactoriamente!!!");
...
function onAgregar() {
	var datos = $('#formTipodiscapacidad').serialize();
	console.log(datos, "agregar");
	$.ajax({
		url: "/uaed/Tipodiscapacidad/registrar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("activar:") != 0){
				...
				var fila = JSON.parse(data);
				agregarFila(fila);
				mostrarMensajeExito("Tipodiscapacidad incluido satisfactoriamente!!!");
			}else if (data.indexOf("error:") != 0){
				mostrarMensajeError(data["error"]);
			}
...
function onEditar() {
	...
	console.log(datos);
	$.ajax({
		url: "/eaed/Tipodiscapacidad/editar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("error:") != 0) {
				...
				agregarFila(cargo);
				mostrarMensajeExito("Tipodiscapacidad modificado satisfactoriamente!!!");
			}
			else {
...
function onEliminar(id) {
	...
	$.ajax({
		url: "/uaed/Tipodiscapacidad/borrar",
		type: "POST",
		data: {"codigo" : id},
		success: function(data) {
			console.log(data);
			if (data.indexOf("codigo:") != 0) {
				...
				mostrarMensajeExito('Tipodiscapacidad eliminado satisfactoriamente!!!');
			}
			else {
