        ...
        [
            {
                ...
                title: 'Listado datos maestros de los Tipos de requerimiento de discapacidad activos',
                filename: 'Listado Tiporequerimiento',
                exportOptions:
                    {
                        columns: [0, 1, 2]
                    }
            },
            {
                ...
                title: 'Listado datos maestros de los Tipos de requerimientos activos',
                filename: 'Listado Tipos de requerimiento',
                exportOptions:
                    {
                        columns: [0, 1, 2]
                    }
            ...
            },
            {
                ...
                title: 'Listado datos maestros de los Tipos de requerimiento de discapacidad activos',
                text: 'Listado Tipos de requerimiento',
                exportOptions:
                    {
                       columns: [0, 1, 2]
                    }
...
function onIncluir() {
	...
	codigo();
	//<?php header('location: /uaed/Tiporequerimiento/codigo'); ?>
	$('#descrp').val("");
	$('#nombre').val("");
...
function codigo() 
    { 
        ...
        var request = $.ajax
            ({
                url: "/uaed/Tiporequerimiento/codigo", //¿Ese archivo se llama así realmente?
                method: "POST"
...
function activar(datos){
	$.ajax({
		url: "/uaed/Tiporequerimiento/activar",
		type: "POST",
		data: datos,
...
function onAgregar() {
	var datos = $('#formTiporequerimiento').serialize();
...
function onEditar() {
	...
	console.log(datos);
	$.ajax({
		url: "/eaed/Tiporequerimiento/editar",
		type: "POST",
		data: datos,
		success: function(data) {
			if (data.indexOf("error:") != 0) {
				...
				agregarFila(cargo);
				mostrarMensajeExito("Tiporequerimiento modificado satisfactoriamente!!!");
			}
			else {
...
function onEliminar(id) {
	...
	$.ajax({
		url: "/uaed/Tiporequerimiento/borrar",
		type: "POST",
		data: {"codigo" : id},
		success: function(data) {
			console.log(data);
			if (data.indexOf("codigo:") != 0) {
				...
				mostrarMensajeExito('Tiporequerimiento eliminado satisfactoriamente!!!');
			}
			else {
